...
import { Category, SelectedWorktime, CreateRecurrenceRule } from '../types/worktime';
...
export function useTimerForm({
  ...
  const { mutate: submitWorktime, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      setSnackBar('info', !data.endTime ? "Temps de travail commencé. Bon travail !" : isEditing ? 'Activité modifiée' : 'Temps enregistré');
